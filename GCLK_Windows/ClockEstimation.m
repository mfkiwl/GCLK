function varargout = ClockEstimation(varargin)
% CLOCKESTIMATION MATLAB code for ClockEstimation.fig
%      CLOCKESTIMATION, by itself, creates a new CLOCKESTIMATION or raises the existing
%      singleton*.
%
%      H = CLOCKESTIMATION returns the handle to a new CLOCKESTIMATION or the handle to
%      the existing singleton*.
%
%      CLOCKESTIMATION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CLOCKESTIMATION.M with the given input arguments.
%
%      CLOCKESTIMATION('Property','Value',...) creates a new CLOCKESTIMATION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ClockEstimation_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ClockEstimation_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ClockEstimation

% Last Modified by GUIDE v2.5 25-May-2024 16:53:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ClockEstimation_OpeningFcn, ...
                   'gui_OutputFcn',  @ClockEstimation_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ClockEstimation is made visible.
function ClockEstimation_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ClockEstimation (see VARARGIN)

% Choose default command line output for ClockEstimation
...
% UIWAIT makes ClockEstimation wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ClockEstimation_OutputFcn(hObject, eventdata, handles) 
...
%%%---main step gclk
command=strcat(pth,'/dos_bin/gclk.exe',32,'olist -clk',32,fbrd_clk,32,'-sp3',32,fcmb_sp3,32,'+conf',32,fconf,32,'-intv',32,cintv,32,'-fk glonass.fk -sys',32,csys,32,'-nopco');
[status,cmdout] = system(command,'-echo');
fprintf(fileID, '%s', cmdout);
fpil=strcat('pil_',cyear,cdoy);
fpil_clk=strcat('pil',cweek,cdow,'.clk');
command=strcat(pth,'/dos_bin/genclk.exe',32,fpil,32,'-type ASAR');
[status,cmdout] = system(command,'-echo');
fprintf(fileID, '%s', cmdout);
%%% anares
for(i=1:iter)
    ...
    fclose(fid);
    command=strcat(pth,'/dos_bin/gclk.exe',32,'olist -clk',32,fpil_clk,32,'-sp3',32,fcmb_sp3,32,'+conf',32,fconf,32,'-intv',32,cintv,32,'-fk glonass.fk -sys',32,csys,32,'-nopco');
    [status,cmdout] = system(command,'-echo');
    fprintf(fileID, '%s', cmdout);
    command=strcat(pth,'/dos_bin/genclk.exe',32,fpil,32,'-type ASAR');
    [status,cmdout] = system(command,'-echo');
...
if(AR)
    ...
    command=strcat(pth,'/dos_bin/gclk.exe',32,'olist -clk',32,fpil_clk,32,'-sp3',32,fcmb_sp3,32,'+conf',32,fconf,32,'-intv',32,cintv,32,'-amb',32,famb,32,'-fk glonass.fk -sys',32,csys,32,'-nopco');
    [status,cmdout] = system(command,'-echo');
    fprintf(fileID, '%s', cmdout);
    command=strcat(pth,'/dos_bin/genclk.exe',32,fpil,32,'-type AS');
    [status,cmdout] = system(command,'-echo');
    fprintf(fileID, '%s', cmdout);
end
